IF OBJECT_ID('usp_hqms_getbasj','P') IS NOT NULL
DROP PROC usp_hqms_getbasj
GO

/*
生成HQMS数据存储，20200806绩效2.0版本
*/
CREATE PROCEDURE usp_hqms_getbasj
  @ksrq VARCHAR(16),
  @jsrq VARCHAR(16),
  @yydm VARCHAR(2)
...
AS
  ...
  SET @rq1=CONVERT(DATE,LEFT(@ksrq,8),112)
  ...
  /*
  SELECT @dwdm = fvalue FROM dbo.TPARAM WHERE fcode = 'hospitalCode'
  SELECT @dwmc = fvalue FROM tparam WHERE fcode = 'hospitalName'
  */

  ---- 将医院代码传参处理 2022.11.6 add by yangdi
  SELECT @dwdm = fupyydm FROM Tyydmk (NOLOCK) WHERE fyydm = @yydm
  SELECT @dwmc = fname FROM Tyydmk (NOLOCK) WHERE fyydm = @yydm
	...
	AND FCYBS NOT LIKE '黄水%'  AND  FCYBS NOT LIKE '大公馆%'  AND fyydm='01' --and FPRN='20210004383'
...
'' P902,'' p903,FBODYBH P904,CASE WHEN ISNULL(FBLOODBH,'')='' then 6 else FBLOODBH end   P905,
 ...
 substring(convert(varchar(20),fallblood),1,2) F25 , substring(convert(varchar(5),fqtxhs),1,2) F26 ,'1' A20N ,'0' B38 ,REPLACE(FKZRBH,'T',''),REPLACE(FZRDOCTBH,'T',''),REPLACE(FZZDOCTBH,'T',''),REPLACE(FZYDOCTBH,'T',''),REPLACE(FNURSEBH,'T','')
	...
	from #tpatientvisit  
--护理天数	
select a.SYXH,a.TJHL,a.YJHL,a.EJHL,a.SJHL into #basyfb from ZYSVR.CISDB.dbo.EMR_BASYK_FB a (nolock),#tpatientvisit b (nolock)  where  a.SYXH=b.FZYID
...
select SYXH,HY1 ,HY2 ,HY3 ,SXFY ,SXPZ1 ,SXPZ2 ,SXPZ3 ,SXPZ4 ,SXPZ5  into #emrbasyfb from ZYSVR.CISDB.dbo.EMR_BASYK a (nolock),#tpatientvisit b (nolock)  where  a.SYXH=b.FZYID
...
b.fbabynum = 1
 ...
 = a.P30 and isnull(b.fupicdm,'''')<>'''''-- and b.ftype = '''+@icdver+''' '
	...
	SELECT * INTO #C03C_temp from (select a.p3,c.* FROM #Tupdata_hqms  a LEFT JOIN ZYSVR.CISDB.dbo.EMR_BRSYK b  
	

	 ON a.P3=b.BAHM  collate Chinese_PRC_CI_AS
	LEFT JOIN ZYSVR.CISDB.dbo.EMR_BASY_ZDK c ON b.SYXH=c.syxh where isnull(a.p321,'')=''  ) aa 
...
from #Tupdata_hqms a where isnull(a.P831,'')=''
 ...
 update a set P351=c.BLZD ,P352=substring(c.BLZDMC,1,30) from #Tupdata_hqms a,ZYSVR.CISDB.dbo.EMR_BRSYK b,ZYSVR.CISDB.dbo.EMR_BASYK c
...
update #Tupdata_hqms set P7=0,P66=DATEDIFF(DD,convert(datetime,p6)+1,convert(datetime,p22)) where DATEDIFF(DD,convert(datetime,p6)+1,convert(datetime,p22))<365 and DATEDIFF(DD,convert(datetime,p6),convert(datetime,p22))=365
	...
	--职业证书  --未处理
 select id,yszyzbm into #tempzydm from ZYSVR.THIS_ZY.dbo.YY_ZGBMK where isnull(yszyzbm,'')<>''
...
F17,F18,F19,F20,F21,F22,F23,F24,F25,F26,
	...
	--end 其他手术
       ...
       C47,C48x01C,C49x01,C50x01,C48x02C,C49x02,C50x02,C48x03C,C49x03,C50x03,C48x04C,C49x04,C50x04,C48x05C,C49x05,C50x05,
		...
		WHERE a.F17!='-'
		SELECT TOP 788 F18,ISNUMERIC(F18)
		FROM #syjbk_bak
		WHERE F18!='-'AND ISNUMERIC(F18)=0
						*/

		UPDATE #syjbk_bak SET F17='-' WHERE PATINDEX('%[0-9]%',F17)=0 OR ISNUMERIC(F17)=0
		UPDATE #syjbk_bak SET F18='-' WHERE PATINDEX('%[0-9]%',F18)=0 OR ISNUMERIC(F18)=0
		UPDATE #syjbk_bak SET F19='-' WHERE PATINDEX('%[0-9]%',F19)=0 OR ISNUMERIC(F19)=0
		UPDATE #syjbk_bak SET F20='-' WHERE PATINDEX('%[0-9]%',F20)=0 OR ISNUMERIC(F20)=0
		...
		update #syjbk_bak set F19=CASE WHEN CONVERT(NUMERIC(9,2),F19)<=0.0 THEN NULL ELSE CONVERT(VARCHAR(10),CONVERT(INT,ROUND(CONVERT(NUMERIC(9,2),F19),0))) END FROM #syjbk_bak WHERE F19!='-' AND ISNUMERIC(F19)=1
		update #syjbk_bak set F20=CASE WHEN CONVERT(NUMERIC(9,2),F20)<=0.0 THEN NULL ELSE CONVERT(VARCHAR(10),CONVERT(INT,ROUND(CONVERT(NUMERIC(9,2),F20),0))) END FROM #syjbk_bak WHERE F20!='-' AND ISNUMERIC(F20)=1
 ...
 B17,B20,C01C,C02N,F01,F02C,F03N,F04,C03C,C04N,C05C,F05,C06x01C,C07x01N,C08x01C,F06x01,C06x02C,C07x02N,C08x02C,F06x02,C06x03C,C07x03N,C08x03C,F06x03,C06x04C,C07x04N,C08x04C,F06x04,C06x05C,
  C07x05N,C08x05C,F06x05,C06x06C,C07x06N,C08x06C,F06x06,C06x07C,C07x07N,C08x07C,F06x07,C06x08C,C07x08N,C08x08C,F06x08,C06x09C,C07x09N,C08x09C,F06x09,C06x10C,C07x10N,C08x10C,F06x10,C06x11C,
   C07x11N,C08x11C,F06x11,C06x12C,C07x12N,C08x12C,F06x12,C06x13C,C07x13N,C08x13C,F06x13,C06x14C,C07x14N,C08x14C,F06x14,C06x15C,C07x15N,C08x15C,F06x15,C06x16C,C07x16N,C08x16C,F06x16,C06x17C,
	...
	UPDATE #syjbk_bak SET A16=NULL WHERE A14>0 AND A16=0
